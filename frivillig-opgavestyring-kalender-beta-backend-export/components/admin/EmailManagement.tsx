...
export const EmailManagement: React.FC = () => {
    const { settings, setSettings, userHasPermission, roles } = useData();
    ...
    const handleSmtpChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setLocalSettings(prev => ({
            ...prev,
            smtp: { ...prev.smtp, [smtpField]: value }
        }));
    };
    ...
    const handleRoleDefaultChange = (type: NotificationType, roleId: string, checked: boolean) => {
        setLocalSettings(prev => {
            ...
            const newRolesForType = checked
                ? [...currentDefaults[type], roleId]
                : currentDefaults[type].filter(id => id !== roleId);
            
            return {
                ...prev,
                notification_role_defaults: {
                    ...currentDefaults,
                    [type]: newRolesForType
                }
            };
        });
    };
    ...
    const handleSaveSettings = () => {
        ...
        toast.success('Email indstillingerne er gemt!');
    };
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md space-y-8">
            ...
            <div className="space-y-6">
                            ...
                            return (
                                <div key={type} className="p-4 border dark:border-slate-700 rounded-lg">
                                    ...
                                    <div className="mt-2 space-y-2">
                                        <input 
                                            type="text"
                                            value={localSettings.email_templates?.[type]?.subject || ''}
                                        ...
                                        />
                                        <textarea
                                            value={localSettings.email_templates?.[type]?.body || ''}
                                            ...
                                            <div className="flex items-center gap-x-6 gap-y-2 flex-wrap">
                                                {(['to', 'cc', 'bcc'] as const).map(method => (
                                                    <div key={method} className="flex items-center">
                                                        <input
                                                            type="radio"
                                                            id={`delivery-${type}-${method}`}
                                                            name={`delivery-${type}`}
                                                            value={method}
                                                            checked={localSettings.email_templates?.[type]?.deliveryMethod === method}
                                                            onChange={e => handleTemplateChange(type, 'deliveryMethod', e.target.value)}
                                                            className="h-4 w-4 border-slate-300 dark:border-slate-600 text-emerald-600 focus:ring-emerald-500 bg-white dark:bg-slate-700"
                                                        />
                                                        <label htmlFor={`delivery-${type}-${method}`} className="ml-2 block text-sm text-slate-900 dark:text-slate-300">
                                                            {method.toUpperCase()}
                                                        </label>
                                                    </div>
                                                ))}
                                ...
                                <div className="mt-2 flex flex-wrap gap-4">
                                    {roles.map(role => (
                                        <div key={role.id} className="flex items-center">
                                             <input
                                                id={`role-default-${type.id}-${role.id}`}
                                                type="checkbox"
                                                checked={localSettings.notification_role_defaults?.[type.id]?.includes(role.id) ?? false}
                                                onChange={e => handleRoleDefaultChange(type.id, role.id, e.target.checked)}
                ...
                <button onClick={handleSaveSettings} className="w-full bg-emerald-600 text-white p-2 rounded hover:bg-emerald-700">Gem Email Indstillinger</button>
            </div>
            ) : (
